...
entity UC_core_resume is
    port (
        clk            : in  STD_LOGIC;
        rst            : in  STD_LOGIC;
        resume_btn_i   : in  STD_LOGIC;
        ...
        resume_pulse_o : out STD_LOGIC
    );
end UC_core_resume;

architecture Structural of UC_core_resume is
...
begin
    ...
    U_RESUME_SYNC : entity work.Resume_Pulse_Sync
        port map (
            clk_i   => clk,
            rst_i   => rst,
            async_i => resume_btn_i,
            sync_o  => open,
            pulse_o => resume_pulse_s
        );

    U_PC : entity work.PC_resume
        port map (
            clk     => clk,
            reset   => rst,
            PCEn    => pcen_s,
    ...
    pcen_s        <= (not halt_s) or resume_pulse_s;
